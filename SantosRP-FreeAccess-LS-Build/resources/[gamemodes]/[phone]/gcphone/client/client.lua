    ...
    if takePhoto ~= true then
      if IsControlJustPressed(1, KeyOpenClose) then
        ESX.TriggerServerCallback('gcphone:getItemAmount', function(qtty)
          if tonumber(myPhoneNumber) then
            if qtty > 0 then
              TooglePhone()
              TriggerServerEvent('gcPhone:allUpdate')
            else
              ESX.ShowNotification("~r~Vous n'avez pas de téléphone~s~")
            end
          else
            ESX.ShowNotification("~r~Aucune carte SIM liée~s~")
          end
        end, 'phone')
      end
      if menuIsOpen then
        ...
        if useMouse and hasFocus == ignoreFocus then
          ...
          hasFocus = nuiFocus
        elseif useMouse == false and hasFocus then
        ...
        if hasFocus then
          SetNuiFocus(false, false)
          TriggerEvent('gcphone:setValue', false)
          hasFocus = false
        end
      end 
    end
...
function showFixePhoneHelper (coords)
  for number, data in pairs(FixePhone) do
    ...
    if dist <= 2.0 then
      ESX.ShowHelpNotification("~g~" .. data.name .. ' ~o~' .. number .. '~n~~INPUT_PICKUP~~w~ Utiliser')
    ...
    for i, _ in pairs(PhoneInCall) do 
        ...
        if (dist <= 5.0) then
          ...
          if (dist <= soundDistanceMax) then 
            ESX.ShowHelpNotification("~INPUT_PICKUP~ Décrocher")
...
RegisterNetEvent("santos_menuperso:UpdateNumber")
AddEventHandler("santos_menuperso:UpdateNumber", function(_myPhoneNumber)
  myPhoneNumber = _myPhoneNumber
  TriggerServerEvent("gcPhone:allUpdate")
  SendNUIMessage({event = 'updateMyPhoneNumber', myPhoneNumber = myPhoneNumber})
end)
...
AddEventHandler("gcPhone:receiveMessage", function(message)
  ...
  if message.owner == 0 then
    local text = '~y~Nouveau message'
    if ShowNumberNotification == true then
      text = '~y~Nouveau message du ~y~'.. message.transmitter
      for _,contact in pairs(contacts) do
        if contact.number == message.transmitter then
          text = '~y~Nouveau message de ~g~'.. contact.display
          break
        end
      end
    end
...
function TooglePhone() 
  ...
  if menuIsOpen == true then
    TriggerEvent('gcphone:setValue', true)
    PhonePlayIn()
  else
    TriggerEvent('gcphone:setValue', false)
    PhonePlayOut()
