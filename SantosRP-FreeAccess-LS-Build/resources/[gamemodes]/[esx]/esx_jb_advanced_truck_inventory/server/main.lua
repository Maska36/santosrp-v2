...
AddEventHandler('esx_truck_inventory:removeInventoryItem', function(plate, itemName, type, itemCount)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    if plate ~= " " or plate ~= nil or plate ~= "" then
        MySQL.Async.fetchScalar('SELECT `count` FROM truck_inventory WHERE `plate` = @plate AND `item`= @item AND `itemt`= @itemt',
        {
            ...
            if (countincar ~= nil) and countincar >= itemCount then
                ...
                if xPlayer ~= nil then
                    ...
                    elseif type == 'item_account' then
                        ...
                        if itemCount > 0 then
                            xPlayer.addAccountMoney(itemName, itemCount)
                        end
