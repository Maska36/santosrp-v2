...
RegisterServerEvent('esx_policejob:buyPC')
AddEventHandler('esx_policejob:buyPC', function(player, license, price)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_police', function(account)
		if account.money >= price then
			account.removeMoney(price)
      		...
      		TriggerClientEvent('esx:showNotification', _source, '~g~Vous avez acheté un Permis de Chasse a un citoyen.')
		else
			local missingMoney = price - account.money
			TriggerClientEvent('esx:showNotification', _source, '~r~Il vous manque $'..missingMoney)
    	end
  	end)
end)

ESX.RegisterServerCallback('esx_policejob:itemCheck', function(src, cb)
...
AddEventHandler('esx_mrw_annonces:Annonces', function(result)
	...
	if xPlayer.job.name == 'police' then
		TriggerClientEvent('esx_mrw_annonces:ToAnnonces', -1, result)
		TriggerEvent('SendToDiscord', _source, "Logs", "**"..GetPlayerName(_source).."** fait une annonce police: **"..result.."**", 16007897, "admin")
	end
end)	

RegisterServerEvent('esx_pjob:handcuff')
AddEventHandler('esx_pjob:handcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'police' then
		TriggerClientEvent('esx_pjob:handcuff', target)
	end
end)
