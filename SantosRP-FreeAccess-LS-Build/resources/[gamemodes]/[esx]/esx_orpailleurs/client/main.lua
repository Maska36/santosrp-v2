...
function OpenCloakroomMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		elements = {
			...
			{label = _U('vine_clothes_vine'), value = 'orpailleur_wear'},
		},
	},
	function(data, menu)
		menu.close()
		...
		elseif data.current.value == 'orpailleur_wear' then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				if skin.sex == 0 then
...
function OpenOrpailleursActionsMenu()
	...
	function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('society:openBMenu', 'orpailleurs', function(data, menu)
				menu.close()
			end)
		end
	end,
		...
		function(data, menu)
			...
			function(data2, menu2)
				...
				if count > 0 then
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_orpailleurs:putStockItems', itemName, count)
					Citizen.Wait(300)
					OpenPutStocksMenu()
            	else
               		ESX.ShowNotification(_U('quantity_invalid'))
...
AddEventHandler('esx_orpailleurs:hasEnteredMarker', function(zone)
	...
	elseif zone == 'TraitementLingotDor' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs'  then
		CurrentAction     = 'lingot_dor_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	elseif zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone= zone}
	elseif zone == 'OrpailleursActions' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
		CurrentAction     = 'orpailleurs_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	elseif zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	elseif zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
		local playerPed = PlayerPedId()
...
AddEventHandler('esx_orpailleurs:hasExitedMarker', function(zone)
	...
	elseif (zone == 'TraitementLingotDor' or zone == 'TraitementLingotDargent') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
		TriggerServerEvent('esx_orpailleurs:stopTransform')
	elseif (zone == 'TraitementDeCuivre') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
		TriggerServerEvent('esx_orpailleurs:stopTransform')
	elseif (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'orpailleurs' then
				if CurrentAction == 'pepite_dor_harvest' then
					TriggerServerEvent('esx_orpailleurs:startHarvest', CurrentActionData.zone)
				elseif CurrentAction == 'lingot_dor_traitement' then
				...
				elseif CurrentAction == 'orpailleurs_actions_menu' then
