...
RegisterServerEvent('esx_jbs:startWork')
AddEventHandler('esx_jbs:startWork', function(zoneIndex)
...
RegisterServerEvent('esx_jbs:stopWork')
AddEventHandler('esx_jbs:stopWork', function()
	if playersWorking[source] then
		playersWorking[source] = nil
	end
end)

RegisterNetEvent('esx_jbs:caution')
AddEventHandler('esx_jbs:caution', function(cautionType, cautionAmount, spawnPoint, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)
		...
		if cautionAmount <= Config.MaxCaution and cautionAmount > 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					TriggerClientEvent('esx_jbs:spawnJobVehicle', xPlayer.source, spawnPoint, vehicle)
				else
