...
AddEventHandler('esx_jbs:action', function(job, zone, zoneIndex)
	menuIsShowed = true
	...
	elseif zone.Type == 'work' then
		...
		if IsPedOnFoot(playerPed) then
			TriggerServerEvent('esx_jbs:startWork', zoneIndex)
		else
					...
					if IsPedInAnyVehicle(playerPed, false) then
						...
						if playerPed == driverPed then
							if myPlate[plate] then
								...
								TriggerServerEvent('esx_jbs:caution', 'give_back', giveBack, 0, 0)
...
AddEventHandler('esx_jbs:hasExitedMarker', function(zone)
	TriggerServerEvent('esx_jbs:stopWork')
	hintToDisplay = nil
	menuIsShowed = false
	hintIsShowed = false
	isInMarker = false
...
function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	hintToDisplay = nil
	hintIsShowed = false
	TriggerServerEvent('esx_jbs:caution', 'take', vehicleCaution, spawnPoint, vehicle)
end

RegisterNetEvent('esx_jbs:spawnJobVehicle')
AddEventHandler('esx_jbs:spawnJobVehicle', function(spawnPoint, vehicle)
	local playerPed = PlayerPedId()
		...
		if PlayerData.job then
			...
			if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
				if onDuty or currentZone.Type == 'cloakroom' or PlayerData.job.name == 'reporter' then
					TriggerEvent('esx_jbs:action', PlayerData.job.name, currentZone, currentZoneIndex)
				end
			end
			...
			if not isInMarker and hasAlreadyEnteredMarker then
				...
				TriggerEvent('esx_jbs:hasExitedMarker', currentZone)
			end
		end
