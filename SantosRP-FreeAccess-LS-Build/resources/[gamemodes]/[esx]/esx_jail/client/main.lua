...
RegisterNetEvent('esx_jl:jail')
AddEventHandler('esx_jl:jail', function(jailTime)
		...
		Citizen.CreateThread(function()
			...
			while JailTime > 0 and not unjail do
				...
				if JailTime % 120 == 0 then
					TriggerServerEvent('esx_jl:updateRemaining', JailTime)
				end
			...
			-- jail time served
			TriggerServerEvent('esx_jl:unjailTime', -1)
...
RegisterNetEvent('esx_jl:unjail')
AddEventHandler('esx_jl:unjail', function(source)
	unjail = true
	JailTime = 0
	fastTimer = 0
end)
...
AddEventHandler('playerSpawned', function(spawn)
	if IsJailed then
		...
		TriggerServerEvent('esx_jl:checkJail')
	end
end)
