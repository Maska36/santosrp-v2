...
RegisterNetEvent('refreshStreet')
AddEventHandler('refreshStreet', function(streetN)
	streetName = streetN
end)
...
Citizen.CreateThread(function()
	while true do
			...
			if vehicle and not isPlayerWhitelisted then
				...
				ESX.TriggerServerCallback('esx_outlawalert:isVehicleOwner', function(owner)
					if not owner then
						...
						if streetName ~= "" and playerGender ~= -1 then
		...
		elseif IsPedShooting(PlayerPedId()) and not IsPedCurrentWeaponSilenced(PlayerPedId()) and Config.GunshotAlert then
			Citizen.Wait(3000)
