...
local Positions = {
	...
	['Sell'] = { ['hint'] = '[E] Vendre', ['x'] = 969.16375732422, ['y'] = -2107.9033203125, ['z'] = 31.475671768188 },
...
local HuntCar = nil
local Blips = {
	{ name="Revente Chasse", id=141, x=969.16375732422, y=-2107.9033203125, z=31.475671768188},
	{ name="Zone de Chasse", id=141, x=-769.63067626953, y = 5592.7573242188, z = 33.48571395874 }
}
	...
	Citizen.CreateThread(function()
		for index, v in ipairs(Positions) do
			if index ~= 'SpawnATV' then
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Zone de Chasse')
			...
			for index, value in pairs(Positions) do
				if value.hint ~= nil then
					...
					if OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] Arrêter la chasse'
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		HuntCar = CreateVehicle(`blazer`, Positions['SpawnATV'].x, Positions['SpawnATV'].y, Positions['SpawnATV'].z, 169.79, true, false)
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				local Animal = CreatePed(5, `a_c_deer`, value.x, value.y, value.z, 0.0, true, false)
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, "[E] Abattre l'animal", 0.4)
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Vous avez abattu un animal et reçu une viande de ' ..AnimalWeight.. 'kg')

	TriggerServerEvent('esx-q-h:reward', AnimalWeight)

	DeleteEntity(AnimalId)
end

function SellItems()
	TriggerServerEvent('esx-q-h:sell')
end
