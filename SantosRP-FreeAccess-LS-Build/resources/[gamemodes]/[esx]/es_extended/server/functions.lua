...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET job = @job, job_grade = @job_grade, org = @org, org_grade = @org_grade, loadout = @loadout, position = @position WHERE identifier = @identifier', {
	...
	Async.parallel(asyncTasks, function(results)
		TriggerClientEvent('esx:showNotification', xPlayer.source, "📌 Personnage synchronisé !", false, 184)
...
ESX.SavePlayers = function(cb)
	...
	if #xPlayers > 0 then
		...
		print('[es_extended] [^2INFO^7] Saved 0 player cause there is nobody online')
	end
end
