AddEventHandler('es:playerLoaded', function(playerId, player)
	...
	table.insert(tasks, function(cb)
		...
		table.insert(tasks2, function(cb2)
			...
			MySQL.Async.fetchAll('SELECT job, job_grade, org, org_grade, loadout, position FROM users WHERE identifier = @identifier', {
				['@identifier'] = player.getIdentifier()
			}, function(result)
				...
				local org, orggrade = result[1].org, tostring(result[1].org_grade)

				if ESX.DoesOrgExist(org, orggrade) then
					local orgObject, gradeorgObject = ESX.Orgs[org], ESX.Orgs[org].grades[orggrade]
					...
					userData.org.grade        = tonumber(orggrade)
				...
				if result[1].position and result[1].position ~= '' then
					...
					userData.coords = {x = 640.8, y = 136.6, z = 94.8, heading = 205.8}
				end
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job, userData.org, userData.loadout, userData.playerName, userData.coords)
...
RegisterNetEvent('esx:GiveAnInventoryItem')
AddEventHandler('esx:GiveAnInventoryItem', function(target, type, itemName, itemCount)
	local playerId = source
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				TriggerClientEvent('esx:showNotification', playerId, _U('ex_inv_lim', targetXPlayer.name))
			else
				sourceXPlayer.removeInventoryItem(itemName, itemCount)
				targetXPlayer.addInventoryItem   (itemName, itemCount)
				
				TriggerClientEvent('esx:showNotification', playerId, _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name))
				...
				TriggerEvent('SendToDiscord', playerId, "Logs", "**"..sourceXPlayer.name.."** à donné **"..ESX.Items[itemName].label.." x"..itemCount.."** a **"..targetXPlayer.name.."**", 16007897, "inv")
			end

		else
			TriggerClientEvent('esx:showNotification', playerId, _U('imp_invalid_quantity'))
		end
	elseif type == 'item_money' then
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', playerId, _U('gave_money', ESX.Math.GroupDigits(itemCount), targetXPlayer.name))
			...
			TriggerEvent('SendToDiscord', playerId, "Logs", "**"..sourceXPlayer.name.."** à donné **$"..itemCount.."** en **liquide** a **"..targetXPlayer.name.."**", 16007897, "inv")
		else
			TriggerClientEvent('esx:showNotification', playerId, _U('imp_invalid_amount'))
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', playerId, _U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName], targetXPlayer.name))
			...
			TriggerEvent('SendToDiscord', playerId, "Logs", "**"..sourceXPlayer.name.."** à donné **"..itemName.."** **x"..itemCount.."** a **"..targetXPlayer.name.."**", 16007897, "inv")
		else
			TriggerClientEvent('esx:showNotification', playerId, _U('imp_invalid_amount'))
		end
...
RegisterNetEvent('esx:removeAnInventoryItem')
AddEventHandler('esx:removeAnInventoryItem', function(type, itemName, itemCount)
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(source)
