...
ESX.ShowNotification = function(msg, flash, hudColorIndex)
...
ESX.ShowAdvancedNotification = function(sender, subject, msg, textureDict, iconType, flash, hudColorIndex)
	PlaySound(-1, "NAV", "HUD_AMMO_SHOP_SOUNDSET", 0, 0, 1)
...
ESX.UI.Menu.Open = function(type, namespace, name, data, submit, cancel, change, close)
	...
	menu.setElements = function(val)
		menu.data.elements = val
	end
...
ESX.Game.GetPedMugshotTransparent = function(ped)
	if DoesEntityExist(ped) then
...
ESX.Game.GetObjectPlayerIsFreeAimingAt = function(player)
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.color1 then local color1, color2 = GetVehicleColours(vehicle) SetVehicleColours(vehicle, props.color1, color2) end
		if props.color2 then local color1, color2 = GetVehicleColours(vehicle) SetVehicleColours(vehicle, color1, props.color2) end
		if props.pearlescentColor then local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle) SetVehicleExtraColours(vehicle, props.pearlescentColor, wheelColor) end
		if props.wheelColor then local pearlescentColor, wheelColor = GetVehicleExtraColours(vehicle) SetVehicleExtraColours(vehicle, pearlescentColor, props.wheelColor) end
...
ESX.Game.Utils.DrawText2D = function(x, y,width, height, scale, text, r, g, b, a)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item',
		{
			...
			if data1.current.action == 'give' then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
				{
					...
					if type == 'item_weapon' then
						...
						TriggerServerEvent('esx:GiveAnInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, sourceAmmo)
						menu2.close()
						menu1.close()
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
							...
							if quantity ~= nil then
								TriggerServerEvent('esx:GiveAnInventoryItem', GetPlayerServerId(nearbyPlayer), type, item, quantity)
			...
			elseif data1.current.action == 'remove' then
				...
				if type == 'item_weapon' then
					...
					TriggerServerEvent('esx:removeAnInventoryItem', type, item)
					menu1.close()
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('esx:removeAnInventoryItem', type, item, quantity)
							menu2.close()
							menu1.close()
						end
...
AddEventHandler('esx:showNotification', function(msg, flash, hudColorIndex)
	ESX.ShowNotification(msg, flash, hudColorIndex)
end)

RegisterNetEvent('esx:showAdvancedNotification')
AddEventHandler('esx:showAdvancedNotification', function(sender, subject, msg, textureDict, iconType, flash)
	ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash)
end)

RegisterNetEvent('esx:showHelpNotification')
AddEventHandler('esx:showHelpNotification', function(msg, thisFrame, beep, duration, hudColorIndex)
	ESX.ShowHelpNotification(msg, thisFrame, beep, duration, hudColorIndex)
end)
