...
RegisterNetEvent('esx:UpdateMoneyHUD')
AddEventHandler('esx:UpdateMoneyHUD', function()
	TriggerServerEvent('trew_hud_ui:getServerInfo')
end)
...
AddEventHandler('esx:removeAnInventoryItem', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
...
AddEventHandler('esx:createPickup', function(pickupId, label, playerId, type, name, componentss, tintIndex)
...
RegisterNetEvent('ToggleMoneyHUD')
AddEventHandler('ToggleMoneyHUD', function()
	DrawHUD = not DrawHUD
	if DrawHUD then
