...
function OpenCloakroomMenu()
	...
	function(data, menu)
		...
		elseif data.current.value == 'vigne_wear' then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				if skin.sex == 0 then
...
function OpenVigneActionsMenu()
	...
	function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('society:openBMenu', 'vigne', function(data, menu)
				menu.close()
			end)
		end
	end,
...
AddEventHandler('esx_vigneronjob:hasEnteredMarker', function(zone)
	...
	elseif zone == 'TraitementVin' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	elseif zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	elseif zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	elseif zone == 'VigneronActions' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		CurrentAction     = 'vigne_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	elseif zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	elseif zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
		local playerPed = PlayerPedId()
...
AddEventHandler('esx_vigneronjob:hasExitedMarker', function(zone)
	...
	elseif (zone == 'TraitementVin' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'vigne' then
