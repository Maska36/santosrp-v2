...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			table.insert(pricee, ('<span style="color:green;">%s</span>'):format(_U('generic_shopitem', vehicle.price)))
		end
		...
		elseif action == 'announce' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'menuperso_modo_annonce', {
				...
				function (data, menu)
					...
					TriggerServerEvent('esx_vehicleshop:annonce', ContenuReason)
				end,
				function (data, menu)
					menu.close()
				end)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
					...
					if amount == nil then
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							...
							TriggerServerEvent('esxb:sendB', GetPlayerServerId(closestPlayer), 'society_cardealer', _U('car_dealer'), tonumber(data2.value))
						end
					end
				end, function(data2, menu2)
					menu2.close()
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('society:openBMenu', 'cardealer', function(data2, menu2)
				menu2.close()
			end)
...
RegisterServerEvent("esx_wheel:winCar")
AddEventHandler("esx_wheel:winCar", function()
	local playerPed = PlayerPedId()
	
    ESX.Game.SpawnVehicle("p1", { x = 232.436,y = -856.069, z = 29.811 }, 144.6, function (vehicle)
        TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
...
RegisterServerEvent("santos_menuperso:packCar")
AddEventHandler("santos_menuperso:packCar", function(vehName)
	local playerPed = PlayerPedId()
	local playerCoords = GetEntityCoords(playerPed)
	local playerHeading = GetEntityHeading(playerPed)
	
    ESX.Game.SpawnVehicle(vehName, { x = playerCoords.x, y = playerCoords.y, z = playerCoords.z }, playerHeading, function(vehicle)
        TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
