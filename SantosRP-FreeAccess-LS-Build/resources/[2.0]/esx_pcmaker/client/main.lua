...
function OpenCloakroomMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		elements = {
			...
			{label = _U('vine_clothes_vine'), value = 'pcmaker_wear'},
		},
	},
	function(data, menu)
		menu.close()
...
function OpenVigneActionsMenu()
	...
	function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('society:openBMenu', 'pcmaker', function(data, menu)
				menu.close()
			end)
		end
	end,
...
function OpenVehicleSpawnerMenu()
	...
	local elements = {
		...
		{label = 'Véhicule de Travail #3',  value = 'RAPTOR150'},
	}
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_pcmakerjob:getStockItems', function(items)
		local elements = {}
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count',{
         	...
         	function(data2, menu2)
				...
				if count > 0 then
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_pcmakerjob:getStockItem', itemName, count)
					Citizen.Wait(300)
					OpenGetStocksMenu()
            	else
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_pcmakerjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		function(data, menu)
			...
			function(data2, menu2)
				...
				if count > 0 then
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_pcmakerjob:putStockItems', itemName, count)
					Citizen.Wait(300)
					OpenPutStocksMenu()
            	else
               		ESX.ShowNotification(_U('quantity_invalid'))
...
AddEventHandler('esx_pcmakerjob:hasEnteredMarker', function(zone)
	if zone == 'CircuitFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'circuit_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	elseif zone == 'TraitementGPU' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'gpu_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	elseif zone == 'TraitementCM' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'cm_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	elseif zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	elseif zone == 'PCMakerActions' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'pcmaker_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	elseif zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	elseif zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		local playerPed = PlayerPedId()
...
AddEventHandler('esx_pcmakerjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if zone == 'CircuitFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		TriggerServerEvent('esx_pcmakerjob:stopHarvest')
	elseif (zone == 'TraitementGPU' or zone == 'TraitementCM') and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		TriggerServerEvent('esx_pcmakerjob:stopTransform')
	elseif zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
		TriggerServerEvent('esx_pcmakerjob:stopSell')
	end
...
-- Create Blips
function blips()
   if PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_pcmakerjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_pcmakerjob:hasExitedMarker', LastZone)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'pcmaker' then
				if CurrentAction == 'circuit_harvest' then
					TriggerServerEvent('esx_pcmakerjob:startHarvest', CurrentActionData.zone)
				elseif CurrentAction == 'cm_traitement' then
					TriggerServerEvent('esx_pcmakerjob:startTransform', CurrentActionData.zone)
				elseif CurrentAction == 'gpu_traitement' then
					TriggerServerEvent('esx_pcmakerjob:startTransform', CurrentActionData.zone)
				elseif CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_pcmakerjob:startSell', CurrentActionData.zone)
				elseif CurrentAction == 'pcmaker_actions_menu' then
