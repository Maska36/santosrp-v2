...
function OpenVigneActionsMenu()
	...
	function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('society:openBMenu', 'avocat', function(data, menu)
				menu.close()
			end)
		end
	end,
...
function OpenVehicleSpawnerMenu()
	...
	local elements = {
		{label = 'Véhicule de Travail',  value = 'g65'},
	}
	...
	function(data, menu)
		...
		ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, -9.0, function(vehicle)
...
AddEventHandler('esx_avocatjob:hasEnteredMarker', function(zone)
	if zone == 'AvocatActions' and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
		...
		CurrentActionData = {}
	elseif zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	elseif zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
		local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlPressed(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
