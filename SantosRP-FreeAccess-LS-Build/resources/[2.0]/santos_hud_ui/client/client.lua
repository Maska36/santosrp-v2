...
local isToggle = true
RegisterNetEvent('ToggleHUDUI')
AddEventHandler('ToggleHUDUI', function(bool) 
	isToggle = not isToggle
	bool = bool or nil
	if bool == nil then
		SendNUIMessage({ action = 'toggleUi', value = isToggle })
	elseif bool == true or bool == false then
		SendNUIMessage({ action = 'toggleUi', value = bool })
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		TriggerEvent('refreshStreet', streetN)
...
Citizen.CreateThread(function()
	while true do
		...
		if vehicle and IsPedInAnyVehicle(PlayerPedId(), false) and GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() and vehicleIsOn then
			done = not done
...
RegisterNetEvent('SendVoiceDistance')
AddEventHandler('SendVoiceDistance', function(VoiceName)
	SendNUIMessage({ action = 'setVoiceDistance', value = VoiceName })
end)
...
Citizen.CreateThread(function()
	if Config.ui.showWeapons == true then
		while true do
			...
			elseif not IsPedArmed(player, 7) then
				Citizen.Wait(500)
			else
				status['armed'] = false	
			end
...
RegisterNetEvent('gcphone:setValue')
AddEventHandler('gcphone:setValue', function(val)
	isPhoneOpened = val
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if not ESX.UI.Menu.IsOpen("default", "santos_menuperso", "menu_perso") then
			if not isPhoneOpened then
				local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
				if vehicle and GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
					local vehicleClass = GetVehicleClass(vehicle)
					
					-- Vehicle Cruiser
					if IsControlJustPressed(1, Keys[Config.vehicle.keys.cruiser]) and (has_value(vehiclesCars, vehicleClass) == true) then
						local vehicleSpeedSource = GetEntitySpeed(vehicle)
